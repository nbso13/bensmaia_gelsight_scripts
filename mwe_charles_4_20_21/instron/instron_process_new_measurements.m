...
figure; scatter(ratios, slopes, 30, 'MarkerFaceColor', 'r', 'MarkerEdgeColor', 'r','MarkerFaceAlpha', 0.5);
hold on
xlabel("percentage activator"); ylabel("slope");

best_fit = fit(ratios', slopes', 'poly1');
ys = feval(best_fit, ratios);
plot(ratios, ys, 'k', 'LineWidth', 2, 'HandleVisibility', 'off')

x = 5;
% beeswarm finger slopes
bee_swarm(finger_slopes, 5, 'b', length(finger_slopes));
title("Gel stiffness from component ratio, finger stiffness distribution")
...
colors = [1, 0, 0; 0, 0, 1 ];
strings = {'Gel stiffness values'; 'Finger stiffness values'};
...
function [] = rough_av_plotter(gel_structs, trace_num, batch_nums, batch_edges)
...
for i = batch_nums
    ...
    target_structs = gel_structs(:,batch_edges(i):batch_edges(i+1)-1);
    [av_force, av_ind] = plot_rough_av(target_structs, trace_num, handle_vis, col);
    forces{count} = av_force;
    count = count + 1;
end
...
force_mat = forces{1};
for i = 2:(count-1)
    force_mat = [force_mat, forces{i}];
end

all_average_force = mean(force_mat, 2);

%calc percentiles

upper = prctile(force_mat, 95, 2);
lower = prctile(force_mat, 5, 2);
...
gcf;
hold on
% plot(av_ind, all_average_force, 'Color', ...
%     'r', 'LineWidth', ...
%     1.7, 'DisplayName', "mean human trace");
...
function [av_force, av_ind] = plot_rough_av(gel_structs, trace_num, handle_vis, col)
