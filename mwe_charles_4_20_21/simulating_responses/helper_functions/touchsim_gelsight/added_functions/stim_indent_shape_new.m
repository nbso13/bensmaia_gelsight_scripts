function s = stim_indent_shape_new(shape,trace,pin_offset,samp_freq)
% SAME AS OLD FUNCTION BUT ZEROS NEGATIVE TRACE VALUES AT THE END and
% subtracts max pin offset. amplituded implimented via trace (stim ramp for
% example)
...
% make pin_offset so that tip of texture contacts skin
pin_offset = pin_offset - max(pin_offset);
