...
if same_neuron_flag
    if min_area_flag
        ...
        %restrict affs to those in any scanning window for RA and SA.
    ...
    %calculate indices based on correlations
    [max_grand_inds] = define_indices(real_rates, FRs, in_loc_indices, aff_pop, aff_names);
...
function [max_grand_inds] = define_indices(real_rates, FRs, in_loc_indices, aff_pop, aff_names)
be_plottin = 1;
corrs = cell(2,3);
neur_indices = cell(2,3);
for m = 1:2 % for gel and touchsim
    for i = 1:3 % for every aff class
        temp = zeros(size(real_rates{i}, 2), size(FRs{m}{i}, 2));
        for j = 1:size(real_rates{i}, 2) % for every real afferent
            for k = 1:size(FRs{1}{i}, 2) % for every simmed afferent
                tempest = corrcoef(real_rates{i}(:,j), FRs{1}{i}(:,k));
