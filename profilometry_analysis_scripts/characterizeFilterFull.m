function [amp_ratio_mat, amp_fx, amp_fy] = characterizeFilterFull(data_unfilt, data_filt, ...
    vert_line, horiz_line, cax, log, plotflag, max_freq, one_dim)
...
%show surfaces
% disp("showing surfaces...")
% figure
% subplot(1,2,1)
% surf(data_unfilt.profile(1:floor(size_unfilty/2), 1:floor(size_unfiltx/2)), 'LineStyle', 'None');
% title("Unfiltered Surface (1st quadrant)")
% subplot(1,2,2)
% surf(data_filt.profile(1:floor(size_filty/2), 1:floor(size_filtx/2)), 'LineStyle', 'None');
% title("Filtered Surface (1st quadrant)")
...
max_freq_x=max_freq;
max_freq_y =max_freq;
[unfilt_amp_mat, unfilt_fx, unfilt_fy] = freqAnalysisFull(data_unfilt);
[filt_amp_mat, filt_fx, filt_fy] = freqAnalysisFull(data_filt);
...
amp_ratio_mat = filt_amp_mat./unfilt_amp_mat;
view_ratio_mat = amp_ratio_mat(1:length(view_y_unfilt), 1:length(view_x_unfilt));
amp_fx = unfilt_fx;
amp_fy = unfilt_fy;
...
if plotflag
    figure;
    subplot(2,2,1)
    imagesc(view_x_unfilt, view_y_unfilt, view_unfilt_mat);
    ...
    subplot(2,2,2)
    imagesc(view_x_filt, view_y_filt, view_filt_mat);
    ...
    subplot(2,2,3)
    imagesc(view_x_unfilt, view_y_unfilt, view_ratio_mat)
